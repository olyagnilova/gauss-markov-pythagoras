...
\draw[->] (O) -- (T) node[anchor=south west]{\footnotesize$\hat X$};
\draw[->] (O) -- (U) node[anchor=south east]{\footnotesize$\hat Y$};

\draw (0,0.5) ellipse (4.5cm and 1.5cm) node[anchor=west, right=3.1cm]{\footnotesize$\Lin^{\perp}(Z)$};

\draw[->] (O) -- (yc) node[anchor=north east]{\small $\widetilde Y$};
\draw[->] (O) -- (xc) node[anchor=north east]{\small $\widetilde X$};

\draw[->, blue] (T) -- (S) node[midway,below]{\small$\hat u$};
\draw[dashed] (pzx) -- (T);
\draw[->,red] (U) -- (P) node[midway,below]{\small $\hat v$};
\draw[dashed] (pzy) -- (U);
...
\draw[dashed] (T) -- (ycp);
\draw[->, dashed, blue] (ycp) -- (xc) node[midway, below]{\small$\tilde u$};
\draw[dashed] (U) -- (xcp);
\draw[dashed,->,red] (xcp) -- (yc) node[midway,below]{\small$\tilde v$};

\tkzMarkRightAngle[size=0.15](S,pzx,z);
\tkzMarkRightAngle[size=0.15](S,xc,O);
\tkzMarkRightAngle[size=0.15](T,pzx,z);
\tkzMarkRightAngle[size=0.15](S,T,pzx);
\tkzMarkRightAngle[size=0.15](xc,ycp,O);

\tkzMarkRightAngle[size=0.15](P,yc,O);
\tkzMarkRightAngle[size=0.15](U,pzy,z);
\tkzMarkRightAngle[size=0.15](P,pzy,z);
\tkzMarkRightAngle[size=0.15](P,U,pzy);
